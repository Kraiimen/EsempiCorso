...
public class SortingHatProcedural {
    ...
    private static final int HOUSE_SIZE = (int) Math.ceil((STUDENTS.length + 4) / 4); // Math.ceil arrotonda per eccesso
                                                                                      // in numero intero
    private static final String[][] HOUSES = new String[4][HOUSE_SIZE];
    private static final int[] COUNTERS = new int[4]; // Tiene traccia di quante volte ho messo uno studente in quella
                                                      // casa
    ...
    public static void randomize() {
        for (int i = 0; i < 100; i++) {
            ...
            } while (n2 == n1);// riprova a creare il secondo numero se venisse uguale al primo(non vogliamo
                               // scambiare un numero dell'array con se stesso)
    ...
    private static boolean hasRoom(String houseName, boolean fullCapacity) {
        int housePos = Arrays.asList(HOUSE_NAMES).indexOf(houseName);// Crea una lista momentanea a partire dall'array e
                                                                     // chiede l'index
        if (fullCapacity) {
    ...
    private static void assignStudentToHouse(String studentName, String houseName) {
        int housePos = Arrays.asList(HOUSE_NAMES).indexOf(houseName); // Posizione della case nell'array delle case a
