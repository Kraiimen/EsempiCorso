...
public class SortingHatProcedural {
    ...
    private static final int EXTRA_STUDENTS = STUDENTS.length % HOUSE_NAMES.length;// 18/4 resto di 2 che sono gli extra

    //mi dice se ho degli extra oppure no  
    private static final boolean HAS_EXTRA_STUDENTS = EXTRA_STUDENTS != 0;   // se ci da diverso da 0 ci darà true
    ...
    public static void main(String[] args) {
        ...
        for(int i = PERFECT_CLASS_SIZE; i < PERFECT_CLASS_SIZE + EXTRA_STUDENTS; i++) {// assegniamo la 
    ...
    // Funzione per assegnare la destinazione
    private static void assignToDestination(String studentName, String favouriteHouse, boolean fullcapacity) {
        ...
        if(luck == 0 && houseHasSpace) { 
            ...
            String destination = getRandomAvailableHouse(fullcapacity);  // s enon sei fortunato capiti in una casa random che ha spazio
            if(destination == null) {
                System.out.println("ERRORE LOGICO: tutte le case risultano occupate al massimo. Terminiamo il programma");
                return; 
            }
            assignStudentToHouse(studentName, destination); // per invocare la funzione
        }
    ...
    // Funzione per assegnare i prefetti
    ...
    private static String getRandomAvailableHouse(boolean fullCapacity) {
        ...
        int numAvail = 0; 
        int size = fullCapacity ? HOUSE_SIZE : getHouseSize();
